...
from custom_python_ops.composite_ops import powMagnitude, sqrtMagnitude, applyConstantToComplexPolar, applyConstantToComplexCart
...
def inference(images, hyperParam):
  ...
  if hyperParam.poolingFun == 'spectral-pooling':
    def spectralPool(conv, ksize = None, strides = None, padding = None, name = None):
        ...
        conv = conv[:,:,int(width/4):int(width*3/4),int(height/4):int(height*3/4)]
        return tf.transpose(irfft2d(conv), [0, 2, 3, 1])
